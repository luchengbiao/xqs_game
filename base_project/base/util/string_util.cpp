...
#define NOT_SPACE(x) ((x) != 0x20 && ((x) < 0 || 0x1d < (x)))
	...
	void StringSplit(std::string str, std::string pattern, std::vector<std::string>& vec)
	{
		int pos = 0;
		str += pattern;
		int size = str.size();
	...
	void WstringSplit(std::wstring str, std::wstring pattern, std::vector<std::wstring>& vec)
	{
		int pos = 0;
		str += pattern;
		int size = str.size();
