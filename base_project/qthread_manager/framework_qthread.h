...
namespace nbase
{
	class FrameworkQThread;

	struct FrameworkQThreadTlsData
	{
		FrameworkQThread *self;		// A pointer pointed to the thread itself
	...
	class BASE_EXPORT FrameworkQThread : public virtual nbase::SupportWeakCallback, public QBaseThread
	{
		friend class QThreadMap;
		friend class QThreadManager;
	public:
		// custom message loop
		...
		explicit FrameworkQThread(const char* name);

		virtual ~FrameworkQThread();
		...
		bool IsRunning() const { return (QBaseThread::thread_id() != kInvalidQThreadId) ? true : false; }
		...
		static FrameworkQThread* current();
	...
	protected:
		...
		static void InitTlsData(FrameworkQThread *self);
		...
		static FrameworkQThreadTlsData* GetTlsData();
